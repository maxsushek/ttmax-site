...
export interface BaseSpec {
  ...
  thicknessMm?: number;
  /**
   * Офіційні числа Butterfly для основ: «реакція» (пружність, віддача) і «вібрація»
   * (наскільки основа передає відчуття удару в руку). Шкала виробника, порівнювати
   * можна ЛИШЕ між основами Butterfly — як і швидкість/обертання в накладках.
   */
  reaction?: number;
  vibration?: number;
  /** Розмір лопаті, напр. "158×152 мм". */
  sizeMm?: string;
  /** Країна виробництва, напр. "Японія" / "Європа". */
  madeIn?: Localized;
  /** Доступні типи ручки. */
  handles: Handle[];
}
...
export interface GearSpec {
  ...
  sizes?: string[];
  /**
   * Розфарбування моделі — коли той самий товар іде в кількох кольорах.
   *
   * ⚠️ НЕ плутати з `colors: Color[]` на рівні товару: той union це "black" | "red"
   * і описує СТОРОНИ накладки. Тут — назви кольорів інвентарю, локалізовані,
   * бо «блакитний» ≠ «голубой». Рендериться тим самим селектором, що й sizes.
   */
  colorways?: Localized[];
