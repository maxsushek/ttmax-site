...
// Галерея фото товара: главное фото + миниатюры, листание стрелками,
// свайпом на мобиле и клавишами ←/→. Одно фото — рендерит без управления.
"use client";
...
export type GalleryImage = {
  /** Полноразмерный URL (например, 900×900). */// src/components/catalog/ProductGallery.tsx
// Галерея фото товара: главное фото + миниатюры, листание стрелками,
// свайпом на мобиле и клавишами ←/→. Одно фото — рендерит без управления.
//
// Все кадры рендерятся сразу (в видимой области), поэтому грузятся при открытии
// страницы — переключение стрелкой/миниатюрой мгновенное (просто смена видимости),
// без подзагрузки и задержки.
"use client";
...
export type GalleryImage = {
  /** Полноразмерный URL (например, 900×900). */
  url: string;
  /** Уменьшенный URL для миниатюры (например, 160×160). */
  thumb: string;
  alt: string;
};
...
export function ProductGallery({ images }: { images: GalleryImage[] }) {
  ...
  // Прокручиваем активную миниатюру в зону видимости при листании.
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Главное фото — все кадры стопкой, видим только активный */}
      <div
        ...
        {count > 1 && (
          <>
            <button
              ...
              aria-label="Попереднє фото"
              onClick={() => go(active - 1)}
              className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-black/40 text-ink backdrop-blur transition-all hover:border-accent/50 hover:bg-black/60 active:scale-95"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path
                  d="M15 6l-6 6 6 6"
            ...
            <button
              ...
              aria-label="Наступне фото"
              onClick={() => go(active + 1)}
              className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-black/40 text-ink backdrop-blur transition-all hover:border-accent/50 hover:bg-black/60 active:scale-95"
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path
                  d="M9 6l6 6-6 6"
            ...
            <div className="pointer-events-none absolute bottom-3 right-3 rounded-full border border-white/10 bg-black/50 px-2.5 py-1 font-display text-[11px] font-bold tabular-nums text-ink-muted backdrop-blur">
              {active + 1} / {count}
            </div>
          </>
        )}
