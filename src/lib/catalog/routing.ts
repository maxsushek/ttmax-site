...
export function resolveSegments(segments: string[]): CatalogRoute | null {
  ...
  if (segments.length === 3) {
    ...
    if (
      ...
      // Правило індексації картки винесено в productIndexable() — воно спільне з sitemap.ts,
      // щоб два місця не розійшлися (див. коментар у indexability.ts).
      return { kind: "product", brand, category, product, index: productIndexable(product) };
    }
    return null;
  }
...
export function catalogStaticParams(): { segments: string[] }[] {
  ...
  for (const brand of getActiveBrands()) {
    ...
    /**
     * ⚠️ КАРТКИ ТОВАРІВ СВІДОМО НЕ ПРЕРЕНДЕРИМО — рендеряться при першому заході (dynamicParams
     * = true у page.tsx), далі кешуються ISR, як і раніше. Через ліміт Deployment Storage.
     *
     * 1040 карток — це 346 МБ із ~390 МБ кожної збірки (89%). Vercel зберігає до 20 останніх
     * продакшн-деплоїв незалежно від retention-політики → стеля ~8 ГБ при ліміті Hobby 10 ГБ,
     * і 11.09 сховище впиралось у 100%. Без карток збірка ~45 МБ.
     *
     * Чому це НЕ додає ISR-записів: у кожного деплою свій порожній ISR-кеш, і графік Writes
     * показав, що після кожного деплою краулери й так переписують кожну картку заново.
     * Ціна — перший захід на картку після деплою рендериться на льоту (~0,5–1 с), зазвичай це бот.
     *
     * Категорії, серії, бренди лишаються в пререндері: вони важкі за даними, їх мало, і на
     * них приходять люди з пошуку.
     */
  }
...
export function routeTitle(route: CatalogRoute, locale: Locale): string {
  switch (route.kind) {
    ...
    case "product": {
      ...
      // ⚠️ 2) У 4 моделей сама лише назва займає ~58 символів
      // (Harimoto Tomokazu Innerforce Super ZLC, Dimitrij Ovtcharov Innerforce ALC).
      // Попередня версія відкидала спершу гео-хвіст, потім бренд — і давала гірший
      // результат, ніж вихідний: «…Super ZLC — купить» (обрубок без «в Украине»)
      // при все ще 67 символах. Тобто ми і зіпсували фразу, і в поріг не влізли.
      // Тепер, якщо навіть без гео-хвоста не влазимо, чесніше віддати саму назву
      // товару без комерційного хвоста взагалі: модель ціла, обрубка немає,
      // а Google усе одно покаже приблизно 60 символів.
      const nameOnly = short.replace(
        locale === "ua" ? /\s*—\s*купити.*$/ : /\s*—\s*купить.*$/,
        "",
      );
      if (nameOnly.length < short.length) return nameOnly;
