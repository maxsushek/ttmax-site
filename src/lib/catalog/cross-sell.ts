...
import { MARGIN_PRIORITY, PRIORITY_BONUS, MIN_IN_DEGREE } from "@/data/catalog/priority";
...
export function buildCrossSellGraph(pool: CatalogProduct[]): Map<string, string[]> {
  ...
  const orphans = pool
    .filter((p) => MONEY.has(p.categorySlug) && (inDegree.get(p.slug) ?? 0) < MIN_IN_DEGREE)
    // Спершу найбідніші — щоб донори дісталися тим, кому найгірше.
    .sort((a, b) => (inDegree.get(a.slug) ?? 0) - (inDegree.get(b.slug) ?? 0));

  for (const orphan of orphans) {
    // Донори — найрелевантніші сусіди, у яких вхідних із запасом (щоб не забрати
    // останнє посилання в іншого товару й не створити нову сироту).
    ...
    for (const { c } of donors) {
      if ((inDegree.get(orphan.slug) ?? 0) >= MIN_IN_DEGREE) break;
      ...
      const displaced = list[VISIBLE_SLOTS - 1];
      if (displaced && (inDegree.get(displaced) ?? 0) <= MIN_IN_DEGREE) continue;

      list.splice(VISIBLE_SLOTS - 1, 0, orphan.slug);
