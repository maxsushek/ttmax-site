...
import { resolveHitSlugs, homeDefaults, homeSingleDefaults } from "@/lib/homepage/home";
import { HOME_TEXT_BASES, HOME_SINGLE_KEYS, homeKey } from "@/lib/homepage/keys";
...
export default async function AdminHomepagePage() {
  ...
  const sd = homeSingleDefaults();
  for (const key of HOME_SINGLE_KEYS) defaults[key] = sd[key] ?? "";
    ...
    <AdminShell email={admin.email}>
      <main className="mx-auto max-w-[860px] px-5 py-8">
        ...
        <p className="mb-6 text-sm text-[#888]">
          Усі елементи головної: Hero (заголовки, статистика, бейдж під фото), секції, тристрічка
          довіри, форма-CTA, FAQ, блок «Хіти» та контакти у футері (телефон, email, соцмережі).
          Поля показують поточні значення — змінюйте будь-яке. Зберігається лише змінене; «↺»
          повертає поле до стандартного. Усе застосовується без передеплою.
        </p>
