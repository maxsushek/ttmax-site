...
/**
 * ISR: фото з адмінки з'являються без передеплою (кеш медіа інвалідується тегом + revalidate).
 *
 * ⚠️ ПЕРЕГЕНЕРАЦІЯ САМА ПО СОБІ ISR-ЗАПИСУ НЕ КОШТУЄ — коштує лише ЗМІНЕНИЙ вивід.
 *
 * Документація Vercel: "When revalidation runs and the content hasn't changed from the
 * previous version, no ISR write units are incurred." Раніше тут стояло, що кожне
 * відкриття застарілої сторінки — це запис, і вікно 600 → 3600 скоротить записи вшестеро.
 * Механізм інший: записи йшли тому, що вивід НЕСТАБІЛЬНИЙ — ті самі дані, але рядки
 * RSC-потоку щоразу інакше пронумеровані й розставлені (гонка async-компонентів, насамперед
 * метаданих; див. htmlLimitedBots у next.config.ts). Поки вивід нестабільний, частота
 * перегенерацій на практиці = частота записів; стабільний вивід — і повторна перегенерація
 * без зміни даних безкоштовна. Повністю стабільним його зробити не вдалось.
 *
 * Вікно 3600 лишаємо, але вже з іншої причини: кожна перегенерація — це виклик функції й
 * Active CPU (ліміт Hobby 4 год), навіть коли запису немає.
 *
 * ⚠️ ЦЕ ЧИСЛО САМЕ ПО СОБІ НЕ ВИЗНАЧАЄ ВІКНО. Next бере для маршруту МІНІМУМ між ним і
 * revalidate усіх unstable_cache, які рендер викликає (next/dist/server/web/spec-extension/
 * unstable-cache.js), а всі чотири — media/get.ts, content/get.ts, settings/get.ts,
 * catalog/overrides.ts — мають 3600. Доказ у білді: about і blog revalidate не експортують,
 * а в prerender-manifest у них 3600. Тобто поставити тут false чи 86400 — нічого не змінити;
 * міняти треба одночасно тут, на головній і в усіх чотирьох кешах.
 *
 * ⚠️ І перш ніж ставити false: при збої Supabase у рантаймі media/get.ts віддає ПОРОЖНЮ карту,
 * і unstable_cache її запамʼятовує. Зараз це лікується саме за годину; з false — назавжди
 * (весь сайт без фото, товари сховані й у noindex). Спершу збій має кидати виняток, а не
 * кешуватись.
 *
 * Довше вікно зменшує перегенерації лише для сторінок, які відкривають частіше за вікно.
 * Картку товару краулер відкриває раз на ~2 дні — для неї 600, 3600 і 86400 однакові.
 * І ще: ISR-кеш на Vercel ОКРЕМИЙ для кожного деплою, тож після деплою перша перегенерація
 * кожної відкритої сторінки — завжди запис. Менше деплоїв = менше записів.
 *
 * Реальні зміни їдуть НЕ по таймеру, а тегами — revalidateTag(MEDIA_TAG) в api/admin/media,
 * CONTENT_TAG в api/admin/content, PRODUCT_OVERRIDES_TAG в api/admin/overrides. Таймер —
 * лише страховка на випадок змін повз адмінку. Якщо зʼявиться джерело даних, яке пише
 * повз адмінку і повз теги, — не скорочуйте вікно наосліп, а додайте тег на це джерело.
 */
