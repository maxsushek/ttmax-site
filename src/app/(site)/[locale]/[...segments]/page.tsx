 ...
 *
 * ⚠️ БУЛО 600, СТАЛО 3600 — через квоту ISR Writes на Hobby.
 *
 * Цей маршрут пререндерить 1116 URL (578 товарів/категорій × 2 локалі). Кожне відкриття
 * сторінки, старшої за вікно revalidate, — це перегенерація і ЗАПИС в ISR-кеш. Googlebot,
 * YandexBot та AI-краулери обходять увесь каталог регулярно, тож при вікні 10 хвилин
 * виходило 214 000 записів на місяць при ліміті 200 000. Квота вичерпалась.
 *
 * ⚠️ ЧОМУ ЦЕ БЕЗПЕЧНО, А НЕ «фото тепер оновлюються раз на годину»: реальні зміни їдуть
 * НЕ по таймеру, а тегами — revalidateTag(MEDIA_TAG) в api/admin/media, CONTENT_TAG
 * в api/admin/content, PRODUCT_OVERRIDES_TAG в api/admin/overrides. Завантажили фото
 * в адмінці — сторінка оновилась одразу, як і раніше. Таймер тут лише страховка на
 * випадок змін ПОЗА адмінкою (правка в БД руками, зміна курсу/залишків збоку).
 *
 * Тобто розмін такий: страховочне вікно виросло з 10 хвилин до години, записів стало
 * у 6 разів менше. Якщо колись зʼявиться джерело даних, яке пише мимо адмінки і мимо
 * тегів, — не повертайте 600 наосліп, а додайте тег на це джерело.
 */
