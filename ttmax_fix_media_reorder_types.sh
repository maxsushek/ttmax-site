#!/usr/bin/env bash
# ttmax_fix_media_reorder_types.sh — фікс білду: типобезпечний обмін фото у move().
# noUncheckedIndexedAccess не пропускав [a,b]=[b,a]; тепер явний обмін зі звуженням типу.
# Перезаписує 1 файл. Запуск: bash ttmax_fix_media_reorder_types.sh
set -euo pipefail

if [ ! -f package.json ] || [ ! -f 'src/components/admin/MediaManager.tsx' ]; then echo Запустіть у корені ttmax-site.; exit 1; fi
echo "▶ Фікс типів у MediaManager…"
...
if [ "${TTMAX_NO_GIT:-0}" = "1" ]; then echo "▶ TTMAX_NO_GIT=1 — без git.";
  ...
  git add 'src/components/admin/MediaManager.tsx'
  git commit -m "fix(admin): типобезпечний обмін у move() (build fix)" || echo "(нема змін)"
  git push origin HEAD
  echo "✓ Запушено."
fi
echo ""
echo "✅ Білд має пройти. Перестановка фото працює як раніше."
