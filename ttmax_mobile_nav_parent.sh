#!/usr/bin/env bash
# ttmax_mobile_nav_parent.sh — у мобільному меню тепер можна зайти в САМ розділ.
# Першим пунктом кожного підменю додано посилання "Усі <категорія>" -> на сторінку розділу
# (на десктопі поведінка не змінюється). Перезаписує 1 файл.
# Запуск: bash ttmax_mobile_nav_parent.sh | Сухий: TTMAX_NO_GIT=1 bash ttmax_mobile_nav_parent.sh
set -euo pipefail

if [ ! -f package.json ] || [ ! -f 'src/components/layout/MobileMenu.tsx' ]; then echo Запустіть у корені ttmax-site.; exit 1; fi
echo "▶ Додаю доступ до батьківських розділів у мобільне меню…"
...
if [ "${TTMAX_NO_GIT:-0}" = "1" ]; then echo "▶ TTMAX_NO_GIT=1 — без git.";
  ...
  git commit -m "fix(nav): доступ до батьківських розділів у мобільному меню (Усі <категорія>)" || echo "(нема змін)"
  git push origin HEAD
  echo "✓ Запушено."
fi
echo ""
echo "✅ Готово. На мобільному відкрий меню -> розділ -> перший пункт Усі <категорія> веде на сам розділ."
