...
|---|---|---|---|
| **SEO** | hreflang sitemap (uk/ru+x-default); /privacy,/terms; категорійні тексти+FAQ; метадані/canonical/JSON-LD; **експертні картки всіх 43 накладок** | — | Домен ttmax.ua+301+GSC; og:image+Product images/sku; столи, гайди, відгуки→зірки |
...
| **Адмінка** | Auth; CRM лідів (статуси/нотатки/атрибуція/цінність); замовлення; медіа-менеджер; редактори контенту/цін; налаштування; **`/admin/homepage` — усі тексти головної (Hero, категорії, хіти, колекції, тристрічка довіри, CTA-форма, FAQ) UA/RU + менеджер блоку «Хіти», без передеплою** | — | — |
